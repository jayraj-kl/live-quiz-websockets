...
class Quiz {
    ...
    setActiveProblem(problem) {
        ...
        problem.submissions = [];
        IoManager_1.IoManager.getIo().to(this.roomId).emit("problem", { problem, totalProblems: this.problems.length });
        setTimeout(() => { this.sendLeaderboard(); }, PROBLEM_TIME_S * 1000);
    }
    sendLeaderboard() {
        this.currentState = "leaderboard";
        const leaderboard = this.getLeaderboard();
        IoManager_1.IoManager.getIo().emit("leaderboard", { leaderboard });
    }
    next() {
        this.activeProblem++;
        ...
        if (problem) {
            ...
            IoManager_1.IoManager.getIo().to(this.roomId).emit("QUIZ_END", { message: "The quiz has ended!", finalLeaderboard });
